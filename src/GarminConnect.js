...
module.exports = function getGarminWeights(username, password) {
  return new Promise((resolve, reject) => {
    request(garminConnectLogin, function (err, xhr) {
      ...
      request({
        ...
        xhr.headers['set-cookie'].forEach(function (cookie) {
          if (cookie.indexOf('CASTGC') === 0) {
            ...
            request(garminConnectPostAuth + 'ticket=ST-0' + ticket, function (err, xhr) {
              if (err || xhr.statusCode != 200) {
                return reject('Could not get post auth page ' + garminConnectPostAuth + 'ticket=ST-0' + ticket + ' | result: ' + xhr.statusCode);
              }

              request("https://connect.garmin.com/legacy/session", function (err, xhr) {
                if (err || xhr.statusCode != 200) {
                  return reject('Could not get legacy session');
                }

                request(garminConnectWeights, function (err, xhr, body) {
                  if (err || xhr.statusCode != 200) {
